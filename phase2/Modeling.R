...
#list of predictor variables minus MMBAL_Bin
DDA+CASHBK+DIRDEP+NSF+SAV+ATM+CD+IRA+LOC+INV+ILS+MM+MMCRED+MTG+CC+CCPURC+SDB+HMOWN+MOVED+INAREA+INS+BRANCH+RES+ACCTAGE_Bin+DEPAMT_Bin+CHECKS_Bin+NSFAMT_Bin+PHONE_Bin+TELLER_Bin+SAVBAL_Bin+ATMAMT_Bin+POS_Bin+POSAMT_Bin+CDBAL_Bin+IRABAL_Bin+DDABAL_Bin+LOCBAL_Bin+INVBAL_Bin+ILSBAL_Bin+MTGBAL_Bin+CCBAL_Bin+INCOME_Bin+LORES_Bin+HMVAL_Bin+AGE_Bin+CRSCORE_Bin,             
...
int.model <- glm(INS ~ NSF*MTG*ILS*INV*IRA*DDA*TELLER_Bin*CC*ATMAMT_Bin*CHECKS_Bin*MM*CDBAL_Bin*DDABAL_Bin, 
                 data = df, family = binomial(link = "logit"))
